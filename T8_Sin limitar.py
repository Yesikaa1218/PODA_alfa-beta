...
#alfa y beta
def minimax(player, alpha, beta, max_depth=None, depth=0):
    if check_win("O"):
        return -10
    elif check_win("X"):
        return 10
    ...
    if player == "X":
        ...
        for row in range(BOARD_ROWS):
            for col in range(BOARD_COLS):
                if board[row][col] is None:
                    board[row][col] = "X"
                    score = minimax("O", alpha, beta, max_depth, depth+1)
                    board[row][col] = None
        ...
        for row in range(BOARD_ROWS):
            for col in range(BOARD_COLS):
                if board[row][col] is None:
                    board[row][col] = "O"
                    score = minimax("X", alpha, beta, max_depth, depth+1)
                    board[row][col] = None
...
def computer_move():
    ...
    for row in range(BOARD_ROWS):
        for col in range(BOARD_COLS):
            if board[row][col] is None:
                board[row][col] = "X"
                score = minimax("O", alpha, beta, max_depth=None, depth=0)
                board[row][col] = None
                if score > best_score:
                    best_score = score
                    best_row = row
